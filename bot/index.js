  ...
  bot.on('callback_query', async (query) => {
    ...
    if (data === 'admin_orders') {
      ...
      orders.forEach(o => {
        ...
        bot.sendMessage(chatId, `📦 <b>Заказ #${o.order_uuid.substring(0, 8)}</b>

👤 <b>Клиент:</b> ${escapeHtml(o.first_name)} (ID: ${o.telegram_id})
💰 <b>Сумма:</b> ${formatPrice(o.total_amount)}
📊 <b>Статус:</b> ${o.status}

📞 <b>Контакты:</b> ${escapeHtml(o.contact_info) || 'Не указаны'}
📍 <b>Адрес:</b> ${escapeHtml(o.delivery_address) || 'Не указан'}
💬 <b>Комментарий:</b> ${escapeHtml(o.comment) || '—'}
${o.promocode ? `🎁 <b>Промокод:</b> ${escapeHtml(o.promocode)}` : ''}
...
🕐 ${new Date(o.created_at).toLocaleString('ru-RU')}`, {
    ...
    if (data.startsWith('del_product_')) {
      ...
      if (product) {
        ...
        bot.deleteMessage(chatId, msgId);
        // Показываем снова список для удаления
  ...
  bot.on('message', async (msg) => {
    ...
    if (state.type === 'product') {
      if (state.step === 0) {
        state.name = text;
        state.step = 1;
        bot.sendMessage(chatId, '2️⃣ Описание:');
        return;
      }
      if (state.step === 1) {
        state.description = text;
        state.step = 2;
        bot.sendMessage(chatId, '3️⃣ Цена (число):');
        return;
      }
      if (state.step === 2) {
        ...
        state.price = price;
        state.step = 3;
        bot.sendMessage(chatId, '4️⃣ ID категории (1-4):\n1-Жидкости 2-Поды 3-Расходники 4-Наборы');
        return;
      }
      if (state.step === 3) {
        ...
        state.step = 4;
        bot.sendMessage(chatId, '5️⃣ Отправьте ФОТО товара или напишите "пропустить":');
        return;
      }
      if (state.step === 4) {
        // Обработка фото
        if (text && text.toLowerCase() === 'пропустить') {
          ...
          try {
            ...
            console.log('Фото товара:', state.image_url);
            state.step = 6;
          } catch (e) {
            bot.sendMessage(chatId, '❌ Ошибка загрузки фото. Отправьте ещё раз или "пропустить":');
            return;
          }
        } else {
    ...
    if (state.type === 'news') {
      if (state.step === 0) {
        state.title = text;
        state.step = 1;
        bot.sendMessage(chatId, '2️⃣ Текст новости:');
        return;
      }
      if (state.step === 1) {
        state.content = text;
        state.step = 2;
        bot.sendMessage(chatId, '3️⃣ Отправьте ФОТО или напишите "пропустить":');
        return;
      }
      if (state.step === 2) {
        if (text && text.toLowerCase() === 'пропустить') {
          ...
          try {
            const fileLink = await bot.getFileLink(photo.file_id);
            state.image_url = fileLink.href;
            console.log('Фото новости:', state.image_url);
            state.step = 4;
          } catch (e) {
            bot.sendMessage(chatId, '❌ Ошибка загрузки фото. Отправьте ещё раз или "пропустить":');
            return;
      ...
      if (state.step === 4) {
        try {
          db.prepare('INSERT INTO news (title, content, image_url) VALUES (?, ?, ?)')
            .run(state.title, state.content, state.image_url);
    ...
    if (state.type === 'promocode') {
      if (state.step === 0) {
        ...
        state.code = code;
        state.step = 1;
        bot.sendMessage(chatId, '2️⃣ Скидка % (1-100):');
        return;
      }
      if (state.step === 1) {
        ...
        state.discount = disc;
        state.step = 2;
        bot.sendMessage(chatId, '3️⃣ Лимит использований (0 = безлимит):');
        return;
      }
      if (state.step === 2) {
        ...
        try {
          db.prepare('INSERT INTO promocodes (code, discount, max_uses) VALUES (?, ?, ?)')
            .run(state.code, state.discount, state.max_uses);
